...
namespace Maper
{
    class SpectrGrid
    {
        ...
        public SpectrGrid(string path)
        {
            ...
            this.logg = double.Parse(strMas[0].Replace('.',','));
            this.vturb = double.Parse(strMas[1].Replace('.', ','));
            this.abund = double.Parse(strMas[2].Replace('.', ','));
            str = sr.ReadLine();
            strMas = str.Split(stringSeparators, StringSplitOptions.RemoveEmptyEntries);
            this.lambda0 = double.Parse(strMas[0].Replace('.', ','));
            this.deltaLambda = double.Parse(strMas[1].Replace('.', ','));
            this.lambdaNumber = int.Parse(strMas[2].Replace('.',','));
            str = sr.ReadLine();
            strMas = str.Split(stringSeparators, StringSplitOptions.RemoveEmptyEntries);
            this.teffNumber = int.Parse(strMas[0].Replace('.', ','));
            //
            this.teffSet = new double[this.teffNumber];
            for (int i = 0; i < this.teffNumber; i++)
            {
                this.teffSet[i] = double.Parse(strMas[i+1].Replace('.', ','));
            }
            str = sr.ReadLine();
            strMas = str.Split(stringSeparators, StringSplitOptions.RemoveEmptyEntries);
            //
            ...
            {
                this.muSet[i] = double.Parse(strMas[i + 1].Replace('.', ','));
            }
            //
            ...
            for (int t = 0; t < this.teffNumber; t++)
            {
                for (int l = 0; l < this.lambdaNumber; l++)
                {
                    ...
                    for (int m = 0; m < this.muNumber; m++)
                    {
                        this.spectrCont[t][m][l] = double.Parse(strMas[m + 1].Replace('.', ','));
                    }
                    for (int m = 0; m < this.muNumber; m++)
                    {
                        this.spectrLine[t][m][l] = double.Parse(strMas[m + 1+this.muNumber].Replace('.', ','));
                    }
                }
            }
